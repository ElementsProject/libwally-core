...
/*
 * MuSig2 per-input PSBT field types (BIP-373):
 *   0x1a = PSBT_IN_MUSIG2_PARTICIPANT_PUBKEYS  key: type(1) + agg_pubkey(33)
 *   0x1b = PSBT_IN_MUSIG2_PUB_NONCE            key: type(1) + part_pubkey(33) + agg_pubkey(33) [+ leaf_hash(32)]
 *   0x1c = PSBT_IN_MUSIG2_PARTIAL_SIG          key: type(1) + part_pubkey(33) + agg_pubkey(33) [+ leaf_hash(32)]
 *
 ...
 * exercising the deserialization paths for types 0x1a, 0x1b, and 0x1c, with
 * both the key-path (66-byte) and script-path (98-byte) compound key forms.
 */
...
static void fuzz_musig2_field(const uint8_t *fuzz, size_t fuzz_size,
                               uint8_t field_type, size_t key_suffix_len)
{
...
{
    /* Exercise all MuSig2 per-input field type code paths, in both the
     * key-path (participant+aggregate) and script-path (+leaf hash) forms */
    fuzz_musig2_field(data, size, 0x1a, 33); /* PSBT_IN_MUSIG2_PARTICIPANT_PUBKEYS */
    fuzz_musig2_field(data, size, 0x1b, 66); /* PSBT_IN_MUSIG2_PUB_NONCE */
    fuzz_musig2_field(data, size, 0x1b, 98); /* PSBT_IN_MUSIG2_PUB_NONCE + leaf hash */
    fuzz_musig2_field(data, size, 0x1c, 66); /* PSBT_IN_MUSIG2_PARTIAL_SIG */
    fuzz_musig2_field(data, size, 0x1c, 98); /* PSBT_IN_MUSIG2_PARTIAL_SIG + leaf hash */
