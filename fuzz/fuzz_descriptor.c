...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    /* Derive the network from the input so all variants are reachable
     * without running the whole pipeline four times per input */
    ...
    test_fuzz_descriptor(str, networks[size ? data[0] & 3 : 0]);
