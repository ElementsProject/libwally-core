...
def gen_dart_package(funcs, all_funcs):
    ...
    typemap = {
        ...
        'uint64_t'              : ('Uint64', 'int'),
        'char'                  : ('Utf8',   'Int8'),
        'unsigned char'         : ('Uint8',  'Int8'),
        'void'                  : ('Opaque', 'Opaque'),
        'wally_map_verify_fn_t' : ('Pointer<Opaque>', 'Pointer<Opaque>'),
    }
    ...
    def map_arg(arg, n, num_args):
        ...
        if argtype in typemap:
            ffi_type, dart_type = typemap[argtype]
        elif arg.is_struct:
            ffi_type, dart_type = 'Opaque', arg.struct_name
        else:
            assert False, f'ERROR: Unknown argument type "{argtype}"'
        if arg.is_pointer:
            ffi_type = f'Pointer<{ffi_type}>'
            if arg.is_pointer_pointer:
                ffi_type = f'Pointer<{ffi_type}>'
            dart_type = ffi_type
        return ffi_type, dart_type

    ffi, typ = [], []
    for func in funcs:
        num_args = len(func.args)
        ffi_types, dart_types, arg_names = [], [], []
        for i, arg in enumerate(func.args):
            ffi_type, dart_type = map_arg(arg, i, num_args)
            ffi_types.append(ffi_type + ' ' + arg.name)
            dart_types.append(dart_type + ' ' + arg.name)
            arg_names.append(arg.name)

        typ.append(f"  typedef Native{func.name} = Int Function({', '.join(ffi_types)});")
        ffi.append(f"  late final _{func.name}Ptr = _lookup<NativeFunction<Native{func.name}>>('{func.name}');")
        typ.append(f"  typedef Dart{func.name} = int Function({', '.join(dart_types)});")
        ffi.append(f"  late final _{func.name} = _{func.name}Ptr.asFunction<Dart{func.name}>();")
        ffi.append(f"  int {func.name}({', '.join(dart_types)}) {{")
        ffi.append(f"    return _{func.name}({', '.join(arg_names)});")
        ffi.append( "  }")
        ffi.append( "")

    replace_text('src/dart_wrapper/wallycore/lib/src/wallycore_base.dart', ffi,
                 ['// BEGIN AUTOGENERATED', u'// END AUTOGENERATED'])

    replace_text('src/dart_wrapper/wallycore/lib/src/wallycore_base.dart', typ,
                 ['// BEGIN AUTOGENERATED TYPEDEFS', u'// END AUTOGENERATED TYPEDEFS'])
...
if __name__ == "__main__":
    ...
    gen_dart_package(all_funcs, all_names)
