...
class TransactionTests(unittest.TestCase):
    ...
    def test_hash_prevouts(self):
        ...
        for tx_hex, expected in [
            ...
            for i in range(tx.num_inputs):
                txhash, txhash_len = make_cbuffer('00' * 32)
                ret = wally_tx_get_input_txhash(tx, i, txhash, txhash_len)
                self.assertEqual(ret, WALLY_OK)
                txhashes.extend(txhash)
                ret, txin = wally_tx_get_input_index(tx, i)
                self.assertEqual(ret, WALLY_OK)
                indices[i] = txin
    ...
    def test_get_elements_taproot_signature_hash(self):
        ...
        invalid_cases = [
            ...
            [(14, 0xff)],            # Unknown flag(s)
            [(16, None)],            # NULL output
            [(17, 0)],               # Zero length output
            [(17, 33)],              # Incorrect length output
        ]
        for case in invalid_cases:
            args = [tx, index, scripts, assets, values, tapleaf_script, tapleaf_script_len,
                    key_version, codesep_pos, annex, annex_len, genesis, genesis_len,
