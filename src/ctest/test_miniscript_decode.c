...
static bool test_decode_pk(void)
{
    ...
    /* pk_k with a 33-byte compressed pubkey: script = 0x21 <33 bytes> */
    {
        ...
        ret = ms_node_from_script(script, 34, 0, &output);
        CHECK(ret == WALLY_OK);
        CHECK(output != NULL);
    ...
    /* pk_k with a 65-byte uncompressed pubkey: script = 0x41 <65 bytes> */
    {
        ...
        ret = ms_node_from_script(script, 66, 0, &output);
        CHECK(ret == WALLY_OK);
        CHECK(output != NULL);
        CHECK(output->kind == KIND_MINISCRIPT_PK_K);
     ...
     * tapscript case is tested below. */
    {
        ...
        ret = ms_node_from_script(script, 33, 0, &output);
        CHECK(ret == WALLY_EINVAL);
        CHECK(output == NULL);
    }
     ...
     * script = OP_DUP OP_HASH160 0x14 <20 bytes> OP_EQUALVERIFY */
    {
        ...
        script[23] = OP_EQUALVERIFY;
        ret = ms_node_from_script(script, 24, 0, &output);
        CHECK(ret == WALLY_OK);
        CHECK(output != NULL);
    ...
    /* pk_k with a 32-byte x-only key in tapscript context: WALLY_MS_IS_X_ONLY must be set */
    {
        ...
        memset(key, 0xef, 32);
        memcpy(script + 1, key, 32);
        ret = ms_node_from_script(script, 33, WALLY_MINISCRIPT_TAPSCRIPT, &output);
        CHECK(ret == WALLY_OK);
        CHECK(output != NULL);
        CHECK(output->kind == KIND_MINISCRIPT_PK_K);
...
static bool test_satisfy_thresh(void)
{
    ...
    /* Case 1: thresh(2, older(100), s:pk_k(A)): timelock met, sig_A available → SAT */
    {
        ...
        ret = ms_node_from_script(scriptA, sizeof(scriptA), 0, &node);
        CHECK(ret == WALLY_OK);
        CHECK(node != NULL);
        ms_satisfy_node(node, &stfr, false, &sat, &dissat);
        CHECK(sat.witness.kind == MS_WITNESS_STACK);
    ...
    /* Case 5: thresh(3, older(100), s:pk_k(A), s:pk_k(B)): all three met → SAT */
    {
        ...
        ret = ms_node_from_script(scriptB, sizeof(scriptB), 0, &node);
        CHECK(ret == WALLY_OK);
        CHECK(node != NULL);
        ms_satisfy_node(node, &stfr, false, &sat, &dissat);
        CHECK(sat.witness.kind == MS_WITNESS_STACK);
...
static bool test_satisfy_negative(void)
{
    ...
    /* pk_k, no sig — lookup_sig always returns false */
    {
        ...
        ret = ms_node_from_script(script, 34, 0, &node);
        CHECK(ret == WALLY_OK);
        CHECK(node != NULL);
    ...
    /* pk_h, no sig or key — lookup_pkh is NULL */
    {
        ...
        script[23] = OP_EQUALVERIFY;
        ret = ms_node_from_script(script, 24, 0, &node);
        CHECK(ret == WALLY_OK);
        CHECK(node != NULL);
        ms_satisfy_node(node, NULL, false, &sat, &dissat);
        CHECK(sat.witness.kind == MS_WITNESS_IMPOSSIBLE);
