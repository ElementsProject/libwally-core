...
test('BIP38', function(t) {
  ...
  cases.forEach(function (c) {
    var priv_key = new Buffer(c[0], 'hex');
    var passwd = c[1];
    var flags = c[2];
    var expected = c[3];
    ...
    fun_from_priv(priv_key, passwd, flags).then(function (res) {
      if (flags > K_RAW) {
        t.equal(new Buffer(res).toString('hex'), expected.toLowerCase(),
    ...
    if (flags > K_RAW) {
       expectedForToPriv = new Buffer(expected, 'hex');
    } else {
       expectedForToPriv = expected;
    }
    fun_to_priv(expectedForToPriv, passwd, flags).then(function (res) {
      t.equal(new Buffer(res).toString('hex'), priv_key.toString('hex'),
