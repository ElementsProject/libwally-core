...
test('BIP39', function(t) {
    ...
    var mnemonic_bytes = undefined;
    cases.forEach((item) => {
        mnemonic_bytes = new Uint8Array(item[0].match(/.{1,2}/g).map(v => parseInt(v, 16)));
        wally.bip39_mnemonic_from_bytes('en', Buffer.from(mnemonic_bytes)).then((res) => {
            t.equal(res, item[1]);
        }); 
