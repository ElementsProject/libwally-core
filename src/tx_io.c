...
/* Ensure 'm' is integer-indexed with valid items */
static bool txio_validate_map(const struct wally_map *m, size_t num_items,
                              bool (*len_fn)(size_t))
{
...
static void txio_hash_sha_issuances(cursor_io *io, const struct wally_tx *tx)
{
    if (!txio_hash_cached_item(io, TXIO_SHA_ISSUANCES)) {
        ...
        for (size_t i = 0; i < tx->num_inputs; ++i) {
            ...
            hash_commmitment(&ctx, txin->issuance_amount, sizeof(txin->issuance_amount_len));
            hash_commmitment(&ctx, txin->inflation_keys, sizeof(txin->inflation_keys_len));
        }
                                     ...
                                     const struct wally_map *values)
{
    ...
    hash_commmitment(&io->ctx, assets->items[index].value, assets->items[index].value_len);
    hash_commmitment(&io->ctx, values->items[index].value, values->items[index].value_len);
    hash_varbuff(&io->ctx, scripts->items[index].value, scripts->items[index].value_len);
    ...
    if (!(txin->features & WALLY_TX_IS_ISSUANCE))
        ...
        hash_commmitment(&io->ctx, txin->issuance_amount, sizeof(txin->issuance_amount_len));
        hash_commmitment(&io->ctx, txin->inflation_keys, sizeof(txin->inflation_keys_len));
        {
...
{
    uint64_t v;
    memcpy(&v, values->items[index].value, values->items[index].value_len);
    hash_le64(&io->ctx, v);
    hash_varbuff(&io->ctx, scripts->items[index].value, scripts->items[index].value_len);
    ...
    unsigned char *bytes_out, size_t len)
{
    ...
    if (!tx || index >= tx->num_inputs ||
        !txio_validate_map(scripts, tx->num_inputs, script_len_ok) ||
        !values ||
...
#endif
    if (is_elements) {
       if (!txio_validate_map(assets, tx->num_inputs, asset_len_ok) ||
           !txio_validate_map(values, tx->num_inputs, value_len_ok) ||
           !genesis_blockhash)
           return WALLY_EINVAL;
    } else {
       if (!txio_validate_map(values, tx->num_inputs, satoshi_len_ok))
           return WALLY_EINVAL;
        genesis_blockhash = NULL;
        genesis_blockhash_len = 0;
    }
    if (!scriptpubkey_is_p2tr(scripts->items[index].value,
                              scripts->items[index].value_len))
        return WALLY_EINVAL;
