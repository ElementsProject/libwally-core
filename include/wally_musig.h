...
/** Sizes of serializable opaque MuSig2 objects */
 ...
 * public key, as specified by BIP-328.
 *
 ...
 *
 * :param agg_pk: The 33-byte compressed aggregate public key from
 *|    `wally_musig_pubkey_get`. Note BIP-328 requires the full key: its
 *|    parity affects derivation, so x-only keys are not accepted.
 * :param agg_pk_len: Must be ``EC_PUBLIC_KEY_LEN``.
 ...
 * :param session_secrand32_len: Must be 32.
 * :param seckey: Secret key of the signer (optional, can be NULL).
 * :param seckey_len: Must be ``EC_PRIVATE_KEY_LEN`` if seckey is non-NULL, 0 otherwise.
 * :param pub_key: 33-byte compressed public key of this signer (required).
 * :param pub_key_len: Must be ``EC_PUBLIC_KEY_LEN``.
 ...
 * :param msg32_len: Must be ``EC_MESSAGE_HASH_LEN`` if msg32 is non-NULL, 0 otherwise.
 * :param extra_input32: 32-byte extra entropy input (optional, can be NULL).
 * :param extra_input32_len: Must be 32 if extra_input32 is non-NULL, 0 otherwise.
 ...
 * :param seckey: Secret key of the signer (REQUIRED for counter mode).
 * :param seckey_len: Must be ``EC_PRIVATE_KEY_LEN``.
 * :param pub_key: 33-byte compressed public key of this signer (required).
 *|    Must match the public key of ``seckey`` or `WALLY_EINVAL` is returned.
 * :param pub_key_len: Must be ``EC_PUBLIC_KEY_LEN``.
 * :param keyagg_cache: keyagg_cache from wally_musig_pubkey_agg (optional, can be NULL).
 * :param msg32: 32-byte message to be signed, if known (optional, can be NULL).
 * :param msg32_len: Must be ``EC_MESSAGE_HASH_LEN`` if msg32 is non-NULL, 0 otherwise.
 ...
 * :param pub_key: The signer's 33-byte compressed public key.
 * :param pub_key_len: Must be ``EC_PUBLIC_KEY_LEN``.
 * :param cache: The keyagg_cache from wally_musig_pubkey_agg.
 * :param session: The session from wally_musig_nonce_process.
 */
